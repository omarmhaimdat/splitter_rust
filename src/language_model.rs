...
/// Use custom corpus file if provided, otherwise use default corpus file.
...
impl LanguageModel {
    ...
    /// # Examples
    /// ```
    /// use rsplitter::LanguageModel;
    /// let mut lm = LanguageModel::new("corpus.txt".to_string());
    /// let text = "rustisgreat";
    /// let result = lm.split(text.to_string());
